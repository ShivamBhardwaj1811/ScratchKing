...
public class Ticket : MonoBehaviour
{
    ...
    {
        List<IEnumerator> TicketAnimationsRoutines = new List<IEnumerator> { ScoreBoardManager.instance.AnimMainIn.Play(true), ScoreBoardManager.instance.AdditionalPanelsIn.Play(true)};
        StartCoroutine(Utility.runCoroutinesAllAtOnceWaitToFinish(this, TicketAnimationsRoutines, 0.1f));
    }
    ...
    public IEnumerator ProcessOneByOneSection()
    {
        foreach (var item in TicketAreas)
        {
            ...
            if (item.winSettings.type == AreaType.Gold) yield return ScoreBoardManager.instance.AnimNumberIn.Play();
    ...
    public IEnumerator FinishProcessing(bool ShowAnySprite, List<TicketSection> SectionList, WinSettings winSettings)
    {
        ...
        if (winSettings.type == AreaType.Gold) yield return ScoreBoardManager.instance.AnimNumberOut.Play();
