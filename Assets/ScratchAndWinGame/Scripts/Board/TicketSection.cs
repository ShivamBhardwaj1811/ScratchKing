...
public class TicketSection : MonoBehaviour
{
    ...
    {
        if (areaType == AreaType.Gold) ScoreBoardManager.instance.ConnectMoneyPartUI(ShowAnySprite, WinBoardImagesCount, winSettings, WinningSprite);
        if (areaType == AreaType.Money) ScoreBoardManager.instance.ConnectBonusPartUI(BonusText, winSettings);
    }
    ...
    {
        if (areaType == AreaType.Gold) winSettings = new WinSettings(Utility.RandomFromList(SettingsDatabase.instance.GoldPriceList), AreaType.Gold);
